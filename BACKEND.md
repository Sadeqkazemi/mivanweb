...
2. Copy `.env.example` to `.env`. Choose a local database password and generate a random secret with `node -e "console.log(require('crypto').randomBytes(48).toString('base64url'))"`. To enable Google sign-in, create a Google Web OAuth client, set `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET`, and register `http://127.0.0.1:3000/api/auth/callback/google` as an authorized redirect URI. Password registration also requires a verified Resend sending domain, `RESEND_API_KEY`, and `AUTH_EMAIL_FROM`.
...
6. Register through `/login`. Passwords require at least 12 characters. A six-digit email code must be verified before a session is issued. Codes expire after 10 minutes and allow five attempts. No seeded user or administrator password exists.
...
- Email/password registration with mandatory email OTP verification, sign-in, database sessions, session expiration and real logout.
