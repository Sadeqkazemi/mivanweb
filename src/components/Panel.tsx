...
export function PanelShell({ children, active, admin = false }: {
    ...
    return <div className={`${s.shell} ${admin ? s.admin : ""}`}><aside className={`${s.sidebar} ${open ? s.open : ""}`}><Link className={s.logo} href={admin ? "/admin" : "/"} aria-label="Mivan home"><Image src="/images/mivan-logo.png" width={48} height={28} alt="Mivan"/></Link>{admin ? <div className={s.adminBadge}>● SUPER ADMIN</div> : <div className={s.menuLabel}>MENU</div>}<nav className={s.nav}>{(admin ? adminLinks : userLinks).map(([href, label], i) => <Link key={href} href={href} onClick={() => setOpen(false)} aria-current={label === active ? "page" : undefined} className={`${label === active ? s.active : ""} ${admin && i === 6 ? s.separated : ""}`}>{label}</Link>)}</nav><SignOut className={s.signout}/></aside><main className={s.main}><button className={s.mobileMenu} style={open ? {position:"fixed",left:250,top:16,zIndex:40,background:"white"} : undefined} aria-expanded={open} onClick={() => setOpen(!open)}>{open ? "Close menu" : "☰ Menu"}</button>{children}</main></div>;
}
export function ProfileMenu({ admin = false, active = "" }: {
    ...
    setOpen(false); }}><button className={`${s.avatar} ${admin ? s.adminAvatar : ""}`} aria-label="Account menu" aria-expanded={open} onClick={() => setOpen(!open)}><AccountName initial/></button>{open && <div className={s.dropdown}><div className={s.profileBanner}><span className={s.avatar}><AccountName initial/></span><div><b><AccountName/></b><div><AccountEmail/></div></div></div><div className={s.profileMeta}><span className={`${s.pill} ${s.orangePill}`}>{admin ? "SUPER ADMIN" : "FREE"}</span><span>{admin ? "Admin" : "0 pts"}</span></div><nav>{(admin ? adminLinks.slice(0, 6) : userLinks).map(([href, label]) => <Link key={href} href={href} className={active === label ? s.active : ""} onClick={() => setOpen(false)}>•　{label === "Home" ? "Dashboard" : label}</Link>)}</nav><SignOut/></div>}</div>; }
