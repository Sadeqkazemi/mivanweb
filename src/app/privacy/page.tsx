...
export default function PrivacyPage() {
  return <div className={shared.landing}>
    ...
    <main className={`${shared.container} ${styles.page}`}>
      ...
      <section className={styles.rights} aria-labelledby="rights-title"><div className={styles.rightsHeading}><div><p className={styles.eyebrow}>Your rights</p><h2 id="rights-title">What you can ask for</h2></div><p>Six requests, handled from <Link href="/settings">Settings</Link> or by email to <a href="mailto:info@mivanfood.com">info@mivanfood.com</a>. We respond within 30 days.</p></div><div className={styles.rightsGrid}>{rights.map(([title, copy], i) => <article key={title}><span>0{i + 1}</span><div><h3>{title}</h3><p>{copy}</p></div></article>)}</div></section>
      <section className={styles.contact}><div><h2>Questions about your data?</h2><p>Write to <a href="mailto:info@mivanfood.com">info@mivanfood.com</a>, or Mivan, Downtown Dubai, Burj Khalifa Blvd, Office 210, Dubai, UAE.</p></div><div className={styles.actions}><Link href="/contact" className={`${shared.button} ${shared.primary}`}>Contact us</Link><Link href="/terms" className={`${shared.button} ${shared.outline}`}>Read the Terms</Link></div></section>
