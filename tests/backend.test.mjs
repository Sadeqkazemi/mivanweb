...
async function signup(){const email=`test-${randomUUID()}@example.test`;created.push(email);const r=await request('/api/auth/sign-up/email',{body:{name:'Backend Test',email,password:'Secure-test-password-2026!'}});assert.equal(r.status,200,await r.clone().text());return {email,cookie:r.headers.getSetCookie().map(x=>x.split(';')[0]).join('; ')};}
test('authentication, ownership, validation and persistent profile',async()=>{
 ...
 const duplicate=await request('/api/auth/sign-up/email',{body:{name:'Duplicate',email:a.email,password:'Secure-test-password-2026!'}});assert.notEqual(duplicate.status,200);
